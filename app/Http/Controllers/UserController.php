...
class UserController extends Controller
{
    public function ajaxLogin(Request $request)
    {
        ...
        if ($validation->passes()) {
            if (Auth::attempt(['email' => trim($request->email), 'password' => $request->password], $request->has('remember'))) {
                return response()->json(['redirect' => true, 'success' => true], 200);
            } else {
                if (Auth::attempt(['name' => trim($request->email), 'password' => $request->password], $request->has('remember'))) {
                        return response()->json(['redirect' => true, 'success' => true], 200);
                    } else {
    ...
    public function ajaxUpdate(Request $request) {
        $user = Auth::User();
        if (!filter_var($request->input("email"), FILTER_VALIDATE_EMAIL) === true) 
            return response()->json(["errors"=>["Invalid email."]]);

        $emailTaken = User::where("email", "=", $request->input("email"))->count();
        if($emailTaken && $user->email != $request->input("email")) 
            return response()->json(["errors"=>["Another user already used that email."]]);

        $usernameTaken = User::where("name", "=", $request->input("username"))->count();
        if($usernameTaken && $user->name != $request->input("username")) 
                return response()->json(["errors"=>["Another user already used that email."]]);

        if (preg_match('/^[A-Za-z]{1}[A-Za-z0-9]{5,31}$/', $request->input("username")))
            return response()->json(["errors"=>["Invalid Username."]]);
    ...
    public function myFavorites()
    {
        ...
        $movies = Movie::whereIn('id', $favorites)->get();
        return view('user.favorites', ['user' => Auth::User(), "movies"=>$movies]);
    }
    ...
    public function myRated()
    {
        ...
        $movies = Movie::whereIn('id', $ratings)->get();
        return view('user.rated', ['user' => Auth::User(), "movies"=>$movies]);
    }
