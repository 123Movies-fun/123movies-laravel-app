...
class MovieDownloaderController extends Controller
{
  ...
  public function YandexBot($limit = 1) {

    $uploads = DB::table('uploads')
      ->select("uploads.id as upload_id")
      ->join('imdb', 'imdb.id', '=', 'uploads.imdb_id')
      ->where("status", "=", null)
      ->orderBy("imdb.release_date", "DESC")
      ->orderBy("imdb.rating", "DESC")
      ->orderByRaw("FIELD(quality , 'HD', 'SD', 'CAM') ASC")
      ->groupBy("imdb_id")
      ->limit($limit)->get();
    ...
    foreach($uploads as $upload) {
      ...
      $upload = Upload::find($upload->upload_id);
      $yandex = $this->pickYandex();

      $api_data = json_decode($yandex->api_data);
      $phone = $api_data->phone;
      $movie_title = Movie::where("id", $upload->imdb_id)->first()->title;

      echo $upload->ident_id."<br>";
      $upload->status = "pending";
      $upload->save();

      /* Dispatch this driveid->yandex job to chosen cloudbot */
      $server->sendMessageWCurlExec("yandex_download", Array(
        "username"=>$yandex->username, 
        "password"=>$yandex->password,
        "phone"=>trim($phone),
        "movie_title"=>$movie_title,
        "driveid"=>$upload->ident_id,
        "client_id"=>"a7ce3f90d21347df98383c9c66c5b85c",
        "client_password"=>"592474ed4d5441c6ad7ea1ddbe072be3"
      ), $async = true);

      print_r($server->response);
    }
  }

  public function pickYandex() {
    $yandex = Server::where("id", ">=", "1")->where("type", "=", "yandex")->inRandomOrder()->first();
    $count = Upload::where("status", "=", "pending")->where("server_id", "=", $yandex->id)->count();
    if($count) return $this->pickYandex();
      else return $yandex;
  }


  public function YandexDone(Request $request) {
    $ip = $request->ip();
    $server = Server::where("ip", "=", $ip)->first();
    if(!isset($server->id)) return response()->json(['error' => 'Server not found.'.$ip]);

    $payload = $server->decryptPayload($request->input("data"));
    $payload = json_decode($payload);

    $upload = Upload::where("ident_id", "=", $payload->drive_id)->first();
    $imdb = Movie::find($upload->imdb_id);

    Upload::where('id', "!=", $upload->id)->where("ident_id", "=", $upload->ident_id)->delete();

    if(isset($payload->yandex_username)) {
      $yandex_server = Server::where("username", "=", $payload->yandex_username)->first();
      $yandex_server->yandex_response = (isset($payload->error)) ? $payload->error : "success";
      $yandex_server->disk_usage = $payload->disk_usage;      
      $yandex_server->save();
    }

    if(!isset($payload->success)) {
      $upload->status = $payload->error;
      $upload->save();

      $colors = IRCController::colors();
      $msg = $colors["lightred"].'[Yandex]'.$colors["nc"].' Error Finishing '.$upload->ident_id.' '.json_encode($payload).'';
      IRCController::alert($msg, "#pre");

      //$this->YandexBot($limit = 1);

      return false;
    } else {
      $upload->status = "success";
      //$upload->referral_url = $payload->url;
      $upload->save();
    }

    $imdb->player_status = "success";
    $imdb->save();
    
    $exp = explode("/",$payload->url);
    $ident_id = end($exp);

    $newUpload = $upload->replicate();
    $newUpload->server_id = $yandex_server->id;
    $newUpload->status = "success";
    $newUpload->ident_id = $ident_id;
    $newUpload->referral_url = $payload->url;
    $newUpload->save();

    $colors = IRCController::colors();
    $msg = $colors["blue"].'[Yandex]'.$colors["nc"].' Got Yandex Player URL '.$payload->url.' ('.$payload->drive_id.')';
    IRCController::alert($msg, "#pre");
